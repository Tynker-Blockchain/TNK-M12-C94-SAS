...
currentBlock = None
failedBlocks = []
# Create variable to save the number of miner
totalMiner = 3
...
def miningPool():
    global chain, totalMiner
    #Create the a variable to store the first part of miner name
    minerName = "Miner"

    if request.method == "POST":
        #Check if the request from the mining pool web page is for mining pending transaction. 
        if "miner" in request.form:
            minerAddress = request.form.get("miner")
            chain.minePendingTransactions(minerAddress)

        #Check if the request from the mining pool web page is for adding new miner. 
        elif "newMiner" in request.form:
            #Increment the totalMiner for the second part of miner name 
            totalMiner += 1
            #Concatenate miner name(first part) with the latest number of miner(second part)
            minerName = minerName + str(totalMiner)
            #Create the new miner suing Miner() method
